(function () {
  ...
  (function (sr) {
    ...
    window[sr] = function (fn) {
      ...
      return fn
        ? this.addEventListener("resize", debounce(fn))
  ...
  ready(function () {
    document
      .querySelector("html")
      .classList.add(isMobile() ? "mobile" : "desktop");
    ...
    window.addEventListener("scroll", function () {
      document.querySelectorAll(".mbr-navbar--sticky").forEach(function (el) {
        ...
        if (!el.classList.contains(".mbr-navbar--open"))
          el.classList[method]("mbr-navbar--short");
      });
    });
    ...
    function videoParser(card) {
      outerFind(card, "[data-bg-video]").forEach(function (el) {
        ...
        var img =
          el.querySelector(".mbr-background-video-preview") ||
        ...
        if (
          parsedUrl &&
          (/youtu\.?be/g.test(parsedUrl[3]) || /vimeo/g.test(parsedUrl[3]))
        ) {
          // youtube
          if (parsedUrl && /youtu\.?be/g.test(parsedUrl[3])) {
            ...
            image.onload = function () {
              ...
              if (el.querySelector(".mbr-background-video"))
                el.querySelector(".mbr-background-video").remove();
              ...
              var sectionResolution = {
                ...
                scale:
                  videoElement.parentNode.clientHeight /
                  videoElement.parentNode.clientWidth,
              };
              ...
              var videoResolution = {
                ...
                width:
                  imageResolution.width > sectionResolution.width
                    ? imageResolution.width
                    : sectionResolution.width,
              ...
              if (
                videoResolution.height / videoResolution.width !=
                imageResolution.scale
              ) {
                // if()
                videoResolution.height =
                  videoResolution.width * imageResolution.scale;
              }
              var options = {
              ...
              videoElement.style.marginTop =
                "-" +
                (videoResolution.height - sectionResolution.height) / 2 +
                "px";
              videoElement.style.marginLeft =
                "-" +
                (videoResolution.width - sectionResolution.width) / 2 +
                "px";
            ...
            player.element.style.marginLeft =
              "-" +
              (player.element.scrollWidth - playerParent.scrollWidth) / 2 +
              "px";
    ...
    if (!isBuilder) {
      document.addEventListener("click", function (e) {
        try {
          ...
          if (
            target.parents().some(function (el) {
          ...
          do {
            if (target.hash) {
              ...
              document
                .querySelector(useBody ? "body" : target.hash)
                .forEach(function (el) {
                  e.preventDefault();
                  ...
                  var goTo =
                    target.hash == "#bottom"
                      ? getHeight(el) - window.innerHeight
                      : offset(el).top - stickyMenuHeight;
                  // Disable Accordion's and Tab's scroll
                  if (
                    el.classList.contains("panel-collapse") ||
        ...
        function setNewSize() {
          ...
          if (imgH && imgW) {
            ...
            if (contAspectRatio > aspectRatio) {
              var percent =
                (100 * (contAspectRatio - aspectRatio)) / aspectRatio;
      ...
      $(document).on("add.cards", function (event) {
        ...
        outerFind(event.target, ".mbr-social-likes").forEach(function (el) {
          el.addEventListener(
            ...
            function (event, service, counter) {
              if (counter > 999)
                event.target
      ...
      if (isMobile()) {
        ...
        function elCarouselItem(element) {
          ...
          if (
            ...
            if (
              ...
              element
                .parents(".cloneditem-" + (visibleSlides - 1))
                .some(function (el) {
        ...
        function checkIfInView() {
          ...
          var window_bottom_position =
            window_top_position + window_height - 100;
  ...
  if (!isBuilder && isIE()) {
    $(document).on("add.cards", function (event) {
      ...
      if (
        eventTarget.classList.contains("mbr-slider") ||
        eventTarget.classList.contains("mbr-gallery")
      ) {
        eventTarget
    ...
    if (!isBuilder) {
      ...
      var modal = function (item) {
        ...
        var section = item.parents("section")[0],
          modalWindow = section.querySelector(
            section.querySelector("[data-modal]").getAttribute("data-modal")
          );
    ...
    if (target.querySelector(".carousel-controls"))
      target.querySelectorAll(".carousel-controls").forEach(function (el) {
        el.querySelectorAll("a").forEach(function (el) {
          ...
          bs5
            ? el.setAttribute("data-bs-target", "#" + carouselID)
  ...
  if (!isBuilder) {
    ...
    if (typeof window.initPopupBtnPlugin === "undefined") {
      if (
        document.body.querySelectorAll("section.popup-btn-cards").length != 0
      ) {
  ...
  if (isBuilder) {
    $(document).on(
      "add.cards changeParameter.cards",
      function (event, paramName) {
